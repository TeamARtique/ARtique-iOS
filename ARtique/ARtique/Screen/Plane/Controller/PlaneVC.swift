...
class PlaneVC: UIViewController {
    ...
    @IBAction func createExhibition(_ sender: Any) {
        let exhibitionScene = SCNScene(named: "art.scnassets/ship.scn")!

        if let exhibitionNode = exhibitionScene.rootNode.childNode(withName: "ship", recursively: true),
           let query = sceneView.getRaycastQuery(),
           let result = sceneView.castRay(for: query).first {
            exhibitionNode.position = SCNVector3(
                x: result.worldTransform.columns.3.x,
                y: result.worldTransform.columns.3.y,
                z: result.worldTransform.columns.3.z
            )

            sceneView.scene.rootNode.addChildNode(exhibitionNode)
            isExhibitionStart = true
        }
    }
